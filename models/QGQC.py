...
class PQWGAN_QC():
    ...
    class QuantumCritic(nn.Module):
        ...
        def forward(self, x):
            x = x.view(x.shape[0], -1).to(next(self.parameters()).device)  # Move x para o dispositivo correto
            x = self.qlayer(x)
            sign = torch.sign(x)
            x = torch.abs(x) * torch.pi/2 - 0.001  # Evita infinitos
            x *= sign
            print(torch.tan(x))
            return torch.tan(x)
    ...
    class QuantumGenerator(nn.Module):
        ...
        def forward(self, x):
            x = x.to(next(self.parameters()).device)  # Move x para o dispositivo correto
            ...
            output_images = torch.Tensor(x.size(0), 0).to(x.device)  # Certifica que output_images esteja no dispositivo correto

            for sub_generator_param in self.params:
                patches = torch.Tensor(0, pixels_per_patch).to(x.device)  # Certifica que patches esteja no dispositivo correto
                for item in x:
                    sub_generator_out = self.partial_trace_and_postprocess(item, sub_generator_param).float().unsqueeze(0).to(x.device)  # Certifica que sub_generator_out esteja no dispositivo correto
            ...
            if special_shape:
                final_out = torch.zeros(x.size(0), *self.image_shape).to(x.device)  # Certifica que final_out esteja no dispositivo correto
                for i, img in enumerate(output_images):
                    for patches_done, j in enumerate(range(0, img.shape[0], pixels_per_patch)):
                        patch = torch.reshape(img[j:j+pixels_per_patch], self.patch_shape).to(x.device)
        ...
        def partial_trace_and_postprocess(self, latent_vector, weights):
            ...
            probs_given_ancilla_0 = probs[:2**(self.n_qubits - self.n_ancillas)].to(next(self.parameters()).device)
            post_measurement_probs = probs_given_ancilla_0 / torch.sum(probs_given_ancilla_0)

            # Normaliza a imagem entre [-1, 1]
            post_processed_patch = ((post_measurement_probs / torch.max(post_measurement_probs)) - 0.5) * 2
            return post_processed_patch.to(next(self.parameters()).device)  # Certifica que o patch processado esteja no dispositivo correto
