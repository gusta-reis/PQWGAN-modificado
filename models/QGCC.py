...
class PQWGAN_CC():
    ...
    class QuantumGenerator(nn.Module):
        ...
        def forward(self, x):
            ...
            output_images = torch.Tensor(x.size(0), 0).to(x.device)  # Move x para o dispositivo correto

            for sub_generator_param in self.params:
                patches = torch.Tensor(0, pixels_per_patch).to(x.device)  # Move x para o dispositivo correto
                for item in x:
                    sub_generator_out = self.partial_trace_and_postprocess(item, sub_generator_param).float().unsqueeze(0).to(x.device)  # Move x para o dispositivo correto
            ...
            if special_shape:
                final_out = torch.zeros(x.size(0), *self.image_shape).to(x.device) # Move x para o dispositivo correto
